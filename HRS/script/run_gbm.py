#!/usr/bin/env python3
"""Run gradient boosting classifier for HRS diabetes prediction."""
...
def main() -> None:
    ...
    pd.DataFrame([{"model": "gbm", **classification_metrics(y_test, proba)}]).to_csv(out_dir / "perf.csv", index=False)
    pd.DataFrame({"y_true": y_test.to_numpy(), "gbm": proba}, index=y_test.index).to_csv(out_dir / "pred.csv", index=True)
