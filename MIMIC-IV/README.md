...
This folder contains our demo pipeline for predicting ICU length of stay using the public MIMIC-IV Clinical Database Demo v2.2.
...
Run the scripts from the `MIMIC-IV/` folder.

```bash
/Users/cyixiao/anaconda3/bin/python3 script/prep_data.py
/Users/cyixiao/anaconda3/bin/python3 script/run_lm.py
...
/Users/cyixiao/anaconda3/bin/python3 script/run_quant.py
...
- `script/run_quant.py`: runs quantile regression for median prediction and prediction intervals
- `script/run_nn.py`: runs a PyTorch multilayer perceptron neural network
- `script/calibrate.py`: checks prediction calibration and interval coverage
...
We currently compare these methods:
...
- quantile regression
- PyTorch multilayer perceptron neural network
...
- prediction interval coverage for models that produce intervals
...
The quantile regression script adds uncertainty information through prediction intervals.
...
Additional model outputs are saved by method:

```text
result/lm/       linear regression and elastic net
result/gbm/      gradient boosting
result/rf/       random forest
result/quant/    quantile regression
result/nn/       neural network
result/calib/    calibration and interval coverage summaries
```
