#!/usr/bin/env python3
"""Run a PyTorch MLP model for ICU LOS prediction."""
...
from utils import MODEL_OUT, ROOT, get_xy, load_analytic, make_preprocessor, metrics, save_json
...
def main() -> None:
    ...
    X_fit, X_val, y_fit, y_val = train_test_split(X_train, y_train, test_size=0.25, random_state=SEED, shuffle=True)
    ...
    pred = predict(model, X_test_t)

    out_dir = MODEL_OUT / "nn"
    out_dir.mkdir(parents=True, exist_ok=True)
    pd.DataFrame([{"model": "mlp", **metrics(y_test, pred)}]).to_csv(out_dir / "perf.csv", index=False)
    pd.DataFrame({"y_true": y_test.to_numpy(), "mlp": pred}, index=y_test.index).to_csv(out_dir / "pred.csv", index=True)
